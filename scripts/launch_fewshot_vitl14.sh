...
for dataset in "${datasets[@]}"; do
  for shot in "${shots[@]}"; do
    for seed in "${seeds[@]}"; do
      CUDA_VISIBLE_DEVICES=3 bash coopupl_train_and_val_vit.sh "$dataset" anay_ViT_L_14 end 16 "$shot" False True "$seed" > ~/trainval_logs/vitl14_"$dataset"_"$shot"shots_seed"$seed".txt
    done
  done
done
