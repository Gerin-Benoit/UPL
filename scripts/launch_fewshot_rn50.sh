...
for dataset in "${datasets[@]}"; do
  for shot in "${shots[@]}"; do
    for seed in "${seeds[@]}"; do
      CUDA_VISIBLE_DEVICES=1 bash coopupl_train_and_val.sh "$dataset" rn50_ep50 end 16 "$shot" False True "$seed" > ~/trainval_logs/rn50_"$dataset"_"$shot"shots_seed"$seed".txt
    done
  done
done
